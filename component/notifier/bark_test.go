...
func TestBarkNotifier_DisabledWhenBothEmpty(t *testing.T) {
	log := newSilentLog()
	n := NewBarkNotifier(log, "", "", "", "", "", "", "")
	if n.Enabled() {
		t.Fatal("Enabled() = true, want false when both URLs empty")
	}
	// Calling Send on a disabled notifier must return immediately without error.
	n.Send(context.Background(), outbound.FailoverEvent{Type: outbound.FailoverEventSwitch})
}
