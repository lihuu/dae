...
// NewFailoverController is the legacy single-primary constructor. It wraps
// NewFailoverControllerWithCandidates with a single primary candidate so
// existing single-primary tests and callers keep their exact behavior.
...
// SetEventCallback registers the failover event callback. It must be called
// before traffic begins; changing it during operation is safe under mu. The cb
...
func (fc *FailoverController) runProbe(gen uint64) {
	...
	// If this probe was detached while in flight (reload/rollback/timeout/reset),
	// discard the result. The replacement probe owns state.
	if fc.activeProbe != run || gen != fc.generation || fc.closed {
		return
	}
...
// probeTarget executes the TCP probe against d, using the configured seam or
// production path. Must NOT be called with mu held.
//
// Probe precedence:
//  1. fc.probeTargetTCP(ctx, d) — target-aware test seam. Injected by rotation
//     tests to control results per candidate.
//  2. fc.probeTCP(ctx) — legacy single-primary test seam. Kept so legacy tests
//     that set probeTCP directly continue working. d is ignored because there is
...
func (fc *FailoverController) onProbeSuccessLocked() {
	...
	if fc.recoverySuccesses == 1 {
		...
		}).Info("recovery confirmation started")
	}

	// Move to recovering state if not already there.
	if fc.state != stateRecovering {
