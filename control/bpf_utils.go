...
type loadBpfOptions struct {
	...
	// FakeIP interception: when Enabled, destinations within the IPv4 prefix
	// defined by V4Network/V4Mask are redirected to userspace even for DIRECT.
	FakeIPEnabled   bool
	FakeIPV4Network uint32 // network address in network byte order
	FakeIPV4Mask    uint32 // prefix mask in network byte order
}
