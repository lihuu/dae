...
func newFakeIPTestController(t *testing.T, prefix string, ttl int, apply func(*DnsControllerOption)) *DnsController {
	...
	opt := &DnsControllerOption{
		...
		CacheDeleteCallback: func(string, *DnsCache) error {
			return nil
		},
...
func TestFakeIPAQueryReturnsStableSyntheticAddress(t *testing.T) {
	...
	require.True(t, ok)
	ip1, ok := netip.AddrFromSlice(a1.A)
	require.True(t, ok)
	ip1 = ip1.Unmap()
	require.Equal(t, uint32(60), a1.Hdr.Ttl)

	pfx := netip.MustParsePrefix("198.18.0.0/15")
	require.True(t, pfx.Contains(ip1), "synthetic IP %v must fall within %v", ip1, pfx)

	// Second query for the same domain must return the exact same IP.
	...
	require.True(t, ok)
	ip2, ok := netip.AddrFromSlice(a2.A)
	require.True(t, ok)
	require.Equal(t, ip1, ip2.Unmap(), "subsequent queries for the same domain must return the same synthetic IP")
}

func TestFakeIPAAAAQueryReturnsNODATA(t *testing.T) {
	ctrl := newFakeIPTestController(t, "198.18.0.0/15", 60, nil)

	resp := doFakeIPQuery(t, ctrl, "ipv6.example.com.", dnsmessage.TypeAAAA)
	require.Equal(t, dnsmessage.RcodeSuccess, resp.Rcode,
		"AAAA query for FakeIP domain must succeed (NODATA)")
	require.Empty(t, resp.Answer, "NODATA response must have no answer records")
}

func TestFakeIPNonAddressQueryReturnsNODATA(t *testing.T) {
	ctrl := newFakeIPTestController(t, "198.18.0.0/15", 60, nil)

	for _, qtype := range []uint16{dnsmessage.TypeTXT, dnsmessage.TypeMX, dnsmessage.TypeHTTPS} {
		resp := doFakeIPQuery(t, ctrl, "nonaddr.example.com.", qtype)
		require.Equal(t, dnsmessage.RcodeSuccess, resp.Rcode,
			"qtype=%d for FakeIP domain must return NODATA with RcodeSuccess", qtype)
		require.Empty(t, resp.Answer)
	}
}

func TestFakeIPPoolExhaustionReturnsSERVFAIL(t *testing.T) {
	// /30 has 4 total addresses: .0 (network, skip), .1, .2, .3 (broadcast, skip).
	// Exactly 2 allocatable host addresses.
	ctrl := newFakeIPTestController(t, "198.18.0.0/30", 60, nil)
...
func TestFakeIPCacheEvictionDoesNotDeletePersistentMapping(t *testing.T) {
	...
	ctrl.RemoveDnsRespCacheFamily(baseKeyA)
